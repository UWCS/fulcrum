...
# number of weeks allowed in multi week
NUM_WEEKS = 5

# sizings for multi week
TERM_CONTENT_HEIGHT = POST_HEIGHT / 1.35
TERM_DAY_TEXT_HEIGHT = 75
TERM_DAY_TEXT_SIZE = 50
TERM_PADDING_X = 0.05
TERM_WEEK_PADDING = 0.015
TERM_WEEK_RADIUS = 40
TERM_DAY_SIZE = 0.99
WEEK_NUM_SIZE = 100
WEEK_NUM_WIDTH = 0.1
TERM_DAY_CIRCLE_SIZE = 0.90

# max number of events that can fit in a circle
MAX_EVENTS_IN_CIRCLE = 4
...
def get_event_circle(event: dict, display_location: bool, for_figma: bool) -> svg.G:
    ...
    location = split_text(event["location"], 15) if display_location else []
    ...
    # font sized based on if location is displayed
    if display_location:
        ...
        # grow title and time if no location
        title_size = CIRCLE_SIZE / 7 if len(title) == 1 else CIRCLE_SIZE / 9
        location_size = 0  # not used
        time_size = CIRCLE_SIZE / 9

    # combine into list
    font_sizes = (
        [title_size] * len(title)
        + ([location_size] * len(location) if display_location else [])
        + [time_size]
    )
...
def create_day_circle(events: list[dict], day_size: float, for_figma: bool) -> svg.G:
    ...
    # truncate events to max allowed
    events = events[:MAX_EVENTS_IN_CIRCLE]
...
def create_multi_week(
    ...
    if end.week - start.week + 1 != NUM_WEEKS:
        raise ValueError("Can only create 5 week calendars")
    ...
    # calculate sizings
    term_height = TERM_CONTENT_HEIGHT
    term_width = POST_WIDTH * (1 - 2 * TERM_PADDING_X)
    term_top = POST_HEIGHT - TERM_CONTENT_HEIGHT
    ...
    for i, day in enumerate(day_names):
        elements.append(
            svg.Text(
                text=day,
                x=day_names_base + i * day_size,
                y=term_top + TERM_DAY_TEXT_HEIGHT,
                font_size=TERM_DAY_TEXT_SIZE,
                text_anchor="middle",
                class_=["title"],
            )
        )
    ...
    for i, week in enumerate(events[0]["terms"][0]["weeks"]):
        elements.extend(
            [
                ...
                svg.Rect(
                    x=term_left,
                    y=week_base + i * (week_height + TERM_WEEK_PADDING * term_height),
                    width=week_width,
                    height=week_height,
                    fill=colours["greyer"],
                    rx=TERM_WEEK_RADIUS,
        ...
        # convert days into a single dictionary
        day_map = {day["day"]: day for day in week["days"]}
        all_days = [day_map.get(day, {"day": day, "events": []}) for day in day_names]
        ...
        for j, day in enumerate(all_days):
            ...
            day_circle.transform = [
                svg.Translate(
                    ...
                    text_base + i * (week_height + TERM_WEEK_PADDING * term_height),
                )
            ]
            elements.append(day_circle)
