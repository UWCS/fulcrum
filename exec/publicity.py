...
def get_event_group(shape: list[int], events: dict) -> svg.G:
    ...
    height = CIRCLE_SIZE * rows + SPACING_Y * (rows + 1) + DAY_HEIGHT
    elements = [
    ...
    for i, event in enumerate(day_events):
        ...
        cy = SPACING_Y + row * (CIRCLE_SIZE + SPACING_Y) + CIRCLE_SIZE / 2 + DAY_HEIGHT
...
def create_single_week(events: list[dict], week: Week) -> list[svg.Element]:
    ...
    height = POST_HEIGHT - POST_HEIGHT / 4
        ...
        for y in range(len(grid)):
            for x in range(len(grid[y])):
                if grid[y][x] == i:
                    col, row = x, y
                    break
        offset_x = POST_HEIGHT / len(grid[0])
        offset_y = height / len(grid)
        day.transform = [
            svg.Translate(offset_x * col, POST_HEIGHT - height + offset_y * row)
        ]
        elements.append(day)
