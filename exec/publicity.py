...
def create_multi_week(events: list[dict], start: Week, end: Week) -> list[svg.Element]:
    ...
    week_height = (
        term_height + TERM_DAY_TEXT_HEIGHT - (NUM_WEEKS + 1) * TERM_WEEK_PADDING
    ) / NUM_WEEKS
    ...
    # add week events
    week_base = term_top + TERM_DAY_TEXT_HEIGHT + TERM_WEEK_PADDING
    for i, week in enumerate(events[0]["terms"][0]["weeks"]):
        ...
        elements.append(
            svg.Rect(
                x=term_left,
                y=week_base + i * (week_height + TERM_WEEK_PADDING),
                width=week_width,
                height=week_height,
            )
        )
